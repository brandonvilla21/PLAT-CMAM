<?php
	...
	mysqli_select_db($conexion, $base_de_datos)
        ...
        //Esto hace que en el método de "move_uploaded_file" del archivo subir_foto_php se pueda mandar como nombre de la foto
        //el número de control concatenado con la extensión de la imagen.
        switch ($tipo) {
        case "image/jpg":	$foto = mb_strtoupper($_REQUEST['id_alumno'],'utf-8').".jpg"; 	break;
        case "image/jpeg": $foto = mb_strtoupper($_REQUEST['id_alumno'],'utf-8').".jpg"; 	break;
        case "image/png":  $foto = mb_strtoupper($_REQUEST['id_alumno'],'utf-8').".png"; 	break;
          }
			}
			...
			//Determina si se seleccionó una imagen.
			if ($_FILES['imagen']['name']!='') {
    		//LLama a archivo de subir_foto.php donde se subirá la imagen. La sentencia "mysqli_query" va incluida este archivo también.
				include("subir_foto.php");
			}
			// Si no se seleccionó ningún archivo, continúa normalmente con la alta.
			else {
				//Se ejecuta el query:
